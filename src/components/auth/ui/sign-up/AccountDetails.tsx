...
type Field = "firstName" | "lastName" | "email" | "phoneNumber" | "password";
...
export function AccountDetails({
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = accountDetailsSchema.safeParse(form);
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-4">
        ...
        <div>
          ...
          <TextInput
            ...
            autoComplete="email"
            invalid={Boolean(errors.email || serverErrors?.email)}
          />
          <Hint>
            We&apos;ll send a 4-digit code here to confirm it&apos;s you.
          </Hint>
          <FieldError>{errors.email || serverErrors?.email}</FieldError>{" "}
        </div>
        ...
        <div>
          ...
          <Hint>
            Never shown publicly. Members reach you only once you connect.
          </Hint>
          <FieldError>
        ...
        <div>
          ...
          <div className="mt-2 flex items-center gap-2.5">
            <span className="flex w-19 shrink-0 gap-1" aria-hidden="true">
              {[0, 1, 2].map((index) => (
                <span
                  key={index}
                  className={cn(
                    "h-0.75 flex-1 rounded-full transition-colors duration-300",
                    index < strength.score ? strength.color : "bg-line-strong",
                  )}
                />
              ))}
            </span>

            <span className="text-[12.5px] text-faint">{strength.label}</span>
          </div>
...
function scorePassword(value: string) {
  if (!value.length) {
    return { score: 0, label: "At least 8 characters", color: "" };
  }
  ...
  if (score >= 3) return { score: 3, label: "Strong", color: "bg-jade" };
  if (score === 2) return { score: 2, label: "Good", color: "bg-brand" };

  return {
    score: 1,
    label: "Weak — add length or mixed case",
    color: "bg-danger",
  };
}
